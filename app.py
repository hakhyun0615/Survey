...
def load_data(age_option, gender_option):
    ...
    if (age_option == '전체' and gender_option == '전체'):
        return df
    elif (age_option != '전체' and gender_option == '전체'):
        return df[df['1. 귀하의 연령대는 어떻게 되시나요?'] == age_option]
    elif (age_option == '전체' and gender_option != '전체'):
        return df[df['2. 귀하의 성별은 무엇인가요?'] == gender_option]
    else:
        return df[(df['1. 귀하의 연령대는 어떻게 되시나요?'] == age_option) & (df['2. 귀하의 성별은 무엇인가요?'] == gender_option)]
...
#
see_option = st.sidebar.selectbox('보고 싶은 형식을 선택해주세요.',pd.Series(['표','스타차트']))

#
if (see_option == '표'):
    df = load_data(age_option, gender_option)
    st.dataframe(df)
